...
# Copy configuration.
cp conf_ClickCountNocfl/* flink-cfl/conf
...
for size in 400000000 800000000; do
#for size in 800000000; do
  ...
  for j in {1..3}; do
      ...
      for i in {1..2}; do
        ...
        set +e
        ./flink-cfl/bin/flink run -c nocfl.ClickCountDiffs /home/ggevay/cfl-1.0-SNAPSHOT.jar hdfs://cloud-11:44000/user/ggevay/ClickCountGenerated/0.05/25000000 5 &>$tmpOut
      ...
      set +e
      /usr/bin/time -f ${rtmarker}%e ./flink-cfl/bin/flink run -c nocfl.ClickCountDiffs /home/ggevay/cfl-1.0-SNAPSHOT.jar hdfs://cloud-11:44000/user/ggevay/ClickCountGenerated/0.05/$size 365 &>$tmpOut
      ./check-cfl-success2.sh $tmpOut
      if [ $? != 0 ]; then
        echo "!!!!!!!!!!!!! Flink job failed"
        exit 5

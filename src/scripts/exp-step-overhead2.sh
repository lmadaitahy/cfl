...
for numMachines in {1..25}; do
    ...
    while [ $ok = false ]; do
      ...
      for i in {1..10}; do
        ...
        set +e
        ./flink-cfl/bin/flink run -c $1 /home/ggevay/cfl-1.0-SNAPSHOT.jar 100 &>$tmpOut
      ...
      for i in {1..7}; do
        ...
        set +e
        /usr/bin/time -f ${rtmarker}%e ./flink-cfl/bin/flink run -c $1 /home/ggevay/cfl-1.0-SNAPSHOT.jar 3000 &>$tmpOut
        ./check-cfl-success2.sh $tmpOut
        if [ $? != 0 ]; then
            # The Flink job failed. Stop the Flink cluster and try with this numMachines again.
            echo "Flink job failed. Restarting Flink cluster and trying again."

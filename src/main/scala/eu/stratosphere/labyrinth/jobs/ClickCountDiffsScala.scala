...
object ClickCountDiffsScala {
	...
	private val booleanSer = TypeInformation.of(classOf[Boolean]).createSerializer(new ExecutionConfig)
	...
	def main(args: Array[String]): Unit = {
		...
		// BB 0
		val pageAttributesStream = env.createInput(new TupleCsvInputFormat[Tuple2[Integer, Integer]](new Path(pref + "in/pageAttributes.tsv"), "\n", "\t", typeInfoTupleIntInt)/*, typeInfoTupleIntInt*/).map(new MapFunction[Tuple2[Integer, Integer], TupleIntInt]() {
			@throws[Exception]
		...
		val day_1 = new LabySource[Integer](env.fromCollection(Collections.singletonList(1)), 0, TypeInformation.of(new TypeHint[ElementOrEvent[Integer]]() {})).setParallelism(1)
		// -- Iteration starts here --   BB 1
		val yesterdayCounts_2 = LabyNode.phi("yesterdayCounts_2", 1, new Forward[TupleIntInt](para), tupleIntIntSer, TypeInformation.of(new TypeHint[ElementOrEvent[TupleIntInt]]() {})).addInput(yesterdayCounts_1, false)
		val day_2 = LabyNode.phi("day_2", 1, new Always0[Integer](1), integerSer, TypeInformation.of(new TypeHint[ElementOrEvent[Integer]]() {})).addInput(day_1, false).setParallelism(1)
		...
		val visits_1_tupleized = new LabyNode[Integer, TupleIntInt]("visits_1_tupleized", new FlatMap[Integer, TupleIntInt]() {
			override def pushInElement(e: Integer, logicalInputId: Int): Unit = {
		...
		val clicksMapped = new LabyNode[TupleIntInt, TupleIntInt]("joinedWithAttrs", new JoinTupleIntInt[TupleIntInt]() {
			override protected def udf(b: Int, p: TupleIntInt): Unit = if (b == 0) out.collectElement(TupleIntInt.of(p.f0, 1))
		...
		val notFirstDay = new LabyNode[Integer, Boolean]("notFirstDay", new SingletonBagOperator[Integer, Boolean]() {
			override def pushInElement(e: Integer, logicalInputId: Int): Unit = {
				super.pushInElement(e, logicalInputId)
				out.collectElement(!(e == 1))
			}
		}, 1, new Always0[Integer](1), integerSer, TypeInformation.of(new TypeHint[ElementOrEvent[Boolean]]() {})).addInput(day_2, true, false).setParallelism(1)
		val ifCond = new LabyNode[Boolean, Unit]("ifCond", new ConditionNode(Array[Int](2, 3), Array[Int](3)), 1, new Always0[Boolean](1), booleanSer, TypeInformation.of(new TypeHint[ElementOrEvent[Unit]]() {})).addInput(notFirstDay, true, false).setParallelism(1)
		// -- then branch   BB 2
		...
		val diffs = new LabyNode[TupleIntInt, Integer]("diffs", new OuterJoinTupleIntInt[Integer]() {
			override protected def inner(b: Int, p: TupleIntInt): Unit = out.collectElement(Math.abs(b - p.f1))

			override

			protected def right(p: TupleIntInt): Unit = out.collectElement(p.f1)

			override

			protected def left(b: Int): Unit = out.collectElement(b)
		...
		val notLastDay = new LabyNode[Integer, Boolean]("notLastDay", new SmallerThan(args(1).toInt + 1), 3, new Always0[Integer](1), integerSer, TypeInformation.of(new TypeHint[ElementOrEvent[Boolean]]() {})).addInput(day_3, true, false).setParallelism(1)
		val exitCond = new LabyNode[Boolean, Unit]("exitCond", new ConditionNode(1, 4), 3, new Always0[Boolean](1), booleanSer, TypeInformation.of(new TypeHint[ElementOrEvent[Unit]]() {})).addInput(notLastDay, true, false).setParallelism(1)
